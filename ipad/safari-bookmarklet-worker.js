javascript:(function(){
  /* Canvas AI Helper — Safari Bookmarklet (via Cloudflare Worker relay) */
  /* Replace WORKER_URL below with your actual Cloudflare Worker URL     */
  ...
  document.getElementById('cah-ask').onclick=async function(){
    ...
    try{
      ...
      document.getElementById('cah-ans').innerHTML=data.answer.replace(/&/g,'&amp;').replace(/</g,'&lt;').replace(/>/g,'&gt;').replace(/\n/g,'<br>');
